...
def train(model, model_name, max_depth, l1_weight, loader, epochs, 
            optimizer, device, save_model=False, save_train_state=False):
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(loader, 0):
            ...
            loss_3 = loss_ssim(labels, outputs, max_val=1., kernel_size=7, k1=0.01, k2=0.03)
    ...
    if save_train_state:
        save_state(save_state_path=f'./SavedTrainingState/{model_name}_{current_date_name}', 
                    model=model, 
                    optimizer=optimizer, 
                    epoch=epochs, 
...
def test(model, l1_weight, loader, max_depth, device):
    ...
    with torch.no_grad():
        ...
        print(f"Loss : {(running_loss/total_batch):.4f} RMSE : {running_rmse/total_batch:.4f} REL : {running_rel/total_batch:.4f} ACC^1 : {running_acc_1 /total_batch:.4f} ACC^2 : {running_acc_2/total_batch:.4f} ACC^3 : {running_acc_3/total_batch:.4f}")
